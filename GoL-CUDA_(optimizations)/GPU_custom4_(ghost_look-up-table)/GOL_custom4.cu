...
__global__ void GOL(int dim, int *grid, int *newGrid, int *GPU_rule_table)
{
    // We want id ∈ [1,dim]
    ...
    if (iy <= dim && ix <= dim) {
        ...
        // Use rule_table instead if-else rules.
...
int main(int argc, char* argv[])
{
    ...
    for (iter = 0; iter<maxIter; iter++) {
        ...
        GOL<<<gridSize, blockSize>>>(dim, d_grid, d_newGrid, GPU_rule_table);
