...
int main(int argc, char* argv[])
{
    ...
    for(i = 1; i<=dim; i++) {
	  ...
	  cudaCheckReturn(cudaMalloc((void **) &dev_grid, (1026) * (1026) * sizeof(int)));
	  cudaCheckReturn(cudaMalloc((void **) &dev_newGrid, (1025) * (1026) * sizeof(int)));
	  
	  cudaCheckReturn(cudaMemcpy(dev_grid, grid, (1026) * (1026) * sizeof(int), cudaMemcpyHostToDevice));
